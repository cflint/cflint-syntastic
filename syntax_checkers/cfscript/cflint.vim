...
if exists("g:loaded_syntastic_cfscript_cflint_checker")
    finish
endif

let g:loaded_syntastic_cfscript_cflint_checker = 1

let s:save_cpo = &cpo
set cpo&vim

function! SyntaxCheckers_cfscript_IsAvailable() dict " {{{1
	return executable(expand(self.getExec(), 1))
endfunction " }}}1

"function! SyntaxCheckers_cfscript_GetHighLightRegex(item)
...
function! SyntaxCheckers_cfscript_cflint_GetLocList() dict " {{{1
	...
	if !exists('g:syntastic_cfscript_cflint_sort')
	    let g:syntastic_cfscript_cflint_sort = 1
	endif
